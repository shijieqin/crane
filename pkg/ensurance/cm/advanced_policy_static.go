...
func (p *advancedStaticPolicy) Start(s state.State) error {
	if err := p.validateState(s); err != nil {
		klog.Errorf("[Advancedcpumanager] advanced static policy invalid state: %v, please drain node and remove policy state file", err)
		return err
	}
	return nil
}
...
func (p *advancedStaticPolicy) Allocate(s state.State, pod *v1.Pod, container *v1.Container) error {
	if numCPUs := p.guaranteedCPUs(pod, container); numCPUs != 0 {
		klog.Infof("[Advancedcpumanager] advanced static policy: Allocate (pod: %s, container: %s)", format.Pod(pod), container.Name)
		if _, ok := s.GetCPUSet(string(pod.UID), container.Name); ok {
			klog.Infof("[Advancedcpumanager] advanced static policy: container already present in state, skipping (pod: %s, container: %s)", format.Pod(pod), container.Name)
			return nil
		}
		...
		if err != nil {
			klog.Errorf("[Advancedcpumanager] unable to allocate %d CPUs (pod: %s, container: %s, error: %v)", numCPUs, format.Pod(pod), container.Name, err)
			return err
		}
...
func (p *advancedStaticPolicy) allocateCPUs(s state.State, numCPUs int) (cpuset.CPUSet, error) {
	klog.Infof("[Advancedcpumanager] allocateCpus: (numCPUs: %d)", numCPUs)

	result := cpuset.NewCPUSet()
	klog.Infof("[Advancedcpumanager] allocateCpu %+v", p.assignableCPUs(s))
	...
	klog.Infof("[Advancedcpumanager] allocateCPUs: returning \"%v\"", result)
	return result, nil
}
...
func (p *advancedStaticPolicy) RemoveContainer(s state.State, podUID string, containerName string) error {
	klog.Infof("[Advancedcpumanager] static policy: RemoveContainer (pod: %s, container: %s)", podUID, containerName)
...
func (p *advancedStaticPolicy) NeedAllocated(pod *v1.Pod, container *v1.Container) bool {
...
func (p *advancedStaticPolicy) guaranteedCPUs(pod *v1.Pod, container *v1.Container) int {
	...
	if csp := GetPodCPUSetType(pod, container); csp == CPUSetNone {
		return 0
	}
