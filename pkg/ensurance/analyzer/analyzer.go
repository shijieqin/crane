...
func (s *AnormalyAnalyzer) getEvictExtResourcePods(triggered bool, ac ecache.ActionContext) []executor.EvictPod {
	...
	if triggered {
		...
		allExtCpu := node.Status.Allocatable.Name(v1.ResourceName(fmt.Sprintf(utils.ExtResourcePrefixFormat, v1.ResourceCPU.String())), resource.DecimalSI).MilliValue()
		...
		klog.V(6).Infof("allExtCpu: %s, targetValue: %s, podLen: %s", allExtCpu, targetValue, len(allPods))

		for _, v := range allPods {
			podUseExtCpuValue, podUseExtCpuBool := utils.GetPodRequestExtCpuMilliValue(v)
			if !podUseExtCpuBool {
				continue
			}
			...
			klog.V(6).Infof("allExtCpu: %s, targetValue: %s, pod: %s, podUseExtCpuValue: %s", allExtCpu, targetValue, v.Name, podUseExtCpuValue)
